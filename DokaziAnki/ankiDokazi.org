...
Vemo, da singularna rešitev \(  y = \phi(x) \) in splošna rešitev \(  y = \psi(x, C) \) rešita enačbo \(  y' = f(x, y) \).  Vemo tudi, da je odvod \ref{eq:4} singularne rešitve enak odvodu \ref{eq:3} splošne rešitve v točki \(  x_0 \in J \), ki je konstantna. Odvoda \(  \psi_u \) se pokrajšata med sabo, ko enačimo odvoda in dobimo pogoj, da je
