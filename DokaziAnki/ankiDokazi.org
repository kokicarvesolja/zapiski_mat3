...
\[ \left| \phi_n (x) - \phi_m (x)  \right| < d(\phi_n, \phi_m) < \frac{\epsilon}{2}
\]
...
Za vsak \(  \epsilon \) obstaja \(  N \in \mathbb{N} \) tako, da za vsak \(  n > N \) velja
...
\[ d(\phi_n, \phi_m) = \max_{x \in I\delta} \left| \phi_n - \phi \right| < \epsilon
\]
