...
\[ d(\phi_n, \phi_m) = \max_{x \in I\delta} \left| \phi_n(x) - \phi_m (x) \right|
\]

za poljubna \(  m, n \in \mathbb{N} \)

Dokaz: Definicija maksimum norme <2>3
******* <4>2

\[   \max_{x \in I\delta} \left| \phi_n (x) - \phi_m (x) \right| < \epsilon \implies \left| \phi_n (x) - \phi_m  (x)\right| < \epsilon
\]

za vsaka \(  m, n > N; m, n \in \mathbb{N} \), dovolj velik \(  N \in \mathbb{N} \)
